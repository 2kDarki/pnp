...
class ResolverRemediationTests(unittest.TestCase):
    ...
    def test_index_worktree_mismatch_noninteractive_does_not_prompt(self) -> None:
        h = resolver.Handlers()
        calls: list[list[str]] = []
        ...
        with _runtime_flags(auto_fix=False, dry_run=False, ci=True, interactive=False):
    ...
    def test_index_worktree_mismatch_line_ending_handoff_does_not_recurse_forever(self) -> None:
        h = resolver.Handlers()
        calls: list[list[str]] = []
    ...
    def test_index_worktree_mismatch_old_git_compat_warning_only_is_success(self) -> None:
        h = resolver.Handlers()
        calls: list[list[str]] = []
        ...
        def fake_run(cmd: list[str], cwd: str, check: bool = False,
            ...
            if cmd[:5] == ["git", "-c", "core.autocrlf=false", "add", "-A"]:
                return CompletedProcess(
                    cmd,
                    1,
                    stdout="",
