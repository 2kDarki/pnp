...
class Handlers:
    ...
    def dubious_ownership(self, cwd: str) -> int:
        ...
        prompt = wrap("git reported dubious ownership "
                 ...
                 "directory as safe (git config --global "
                 "--add safe.directory)? [y/n]")
    ...
    def invalid_object(self, stderr: str, cwd: str) -> int:
        ...
        self.warn("git commit failed: encountered invalid "
                + f"object for {file_hint!r}"
        ...
        if not AUTOFIX:
            ...
            choices = {
                ...
                "2": "Attempt hard auto-repair",
                "3": "Skip commit and continue",
                "4": "Abort"
            }
            ...
            self.prompt(auto("attempting hard auto-repair to"
                " resolve missing/dangling blobs"))
            opt = "2"
