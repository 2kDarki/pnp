...
# ======================== LOCALS =========================
from ._constants import DRYRUN, APP, PNP, INFO, GOOD, BAD
...
def bump_semver_from_tag(tag: str, bump: str,
    ...
    if not m:  # start fresh
        if bump == 'patch': return f"{prefix}0.0.1"
...
def main(argv: list[str] | None = None) -> None | NoReturn:
    ...
    if args.dry_run:
        out.raw()
        out.success("dry-run mode: no changes will be made")
    ...
    if gitutils.has_uncommitted(op_path):
        ...
        try:
            ...
            msg = args.tag_message or f"{APP} auto commit"
            if not CI_MODE:
        ...
        try:
            ...
            log_text += utils.gen_changelog(repo_root,
                        since=latest) + "\n"
        except Exception as e:
            hue = BAD
            add = ""
            ...
            log_text += utils.color("changelog generation "
                      + "failed: ", hue) + f"{e}{add}\n"
        
        # log changes
        out.raw(PNP, end="")
        prompt = utils.color("changelog↴\n\n", hue)
        out.raw(wrap(f'{prompt}{log_text}'), end="")
