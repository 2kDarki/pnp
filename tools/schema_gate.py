...
def _extract_json_blob(text: str) -> dict[str, object]:
    ...
    if start == -1 or end == -1 or end <= start:
        raise ValueError("could not locate JSON object in command output")
...
def _validate_payload(name: str, payload: dict[str, object], lock: dict[str, object]) -> list[str]:
    issues: list[str] = []
    required_top = [str(k) for k in lock["required_top_level"]]  # type: ignore[index]
    required_summary = [str(k) for k in lock["required_summary"]]  # type: ignore[index]
    required_item = [str(k) for k in lock["required_item"]]  # type: ignore[index]
